...
class _FeesScreenState extends State<FeesScreen>
  ...
  void initState() {
    ...
    context.read<FeesCubit>().getFees(widget.userId);
  }
    ...
    return BlocBuilder<FeesCubit, FeesState>(
      builder: (context, state) {
        ...
        if (state is FeesLoaded) {
          ...
          final academicFees = state.academicItems;
          final servicesFees = state.servicesItems;
          ...
          return Directionality(
            ...
            child: Scaffold(
              ...
              body: FadeTransition(
                ...
                child: Column(
                  children: [
                    _buildHeader(isAr, isDark, summary),
                    Expanded(
                      child: SingleChildScrollView(
                        padding: const EdgeInsets.all(20),
                        physics: const BouncingScrollPhysics(),
                        child: Column(
                          children: [
                            _buildStatsRow(isAr, isDark, summary),
                            const SizedBox(height: 24),
                            _buildSectionHeader(isAr ? "المصاريف الدراسية" : "Academic Fees", isDark),
                            const SizedBox(height: 12),
                            ...academicFees.map((fee) => _buildFeeRow(fee, isAr, cardColor, textColor, isDark)),
                            const SizedBox(height: 16),
                            _buildSectionHeader(isAr ? "المستلزمات والخدمات" : "Supplies & Services", isDark),
                            const SizedBox(height: 12),
                            ...servicesFees.map((fee) => _buildFeeRow(fee, isAr, cardColor, textColor, isDark)),
                            const SizedBox(height: 24),
                            _buildPayButton(isAr),
                          ],
                        ),
                      ),
                    ),
  ...
  Widget _buildHeader(bool isAr, bool isDark, FeesSummaryEntity summary) {
    return Container(
      decoration: const BoxDecoration(
        gradient: LinearGradient(
            ...
            children: [
              Row(
                children: [
                  ...
                  const Spacer(),
                  Text(
                    isAr ? "السجلات المالية" : "Financial Records",
                    style: const TextStyle(
                      color: Colors.white,
                      fontWeight: FontWeight.bold,
                      fontSize: 17,
              ...
              const SizedBox(height: 24),
              Text(
                isAr ? "إجمالي المبلغ المتبقي" : "Total Remaining",
                style: const TextStyle(color: Colors.white70, fontSize: 14),
              ),
              const SizedBox(height: 6),
              Text(
                "${summary.remainingAmount.toStringAsFixed(0)} EGP",
                style: const TextStyle(
                  color: Colors.white,
                  fontSize: 34,
                  fontWeight: FontWeight.w900,
              ...
              Column(
                children: [
                  Row(
                    ...
                    children: [
                      Text(
                        "${(summary.paidPercentage * 100).toInt()}% ${isAr ? 'مدفوع' : 'Paid'}",
                        style: const TextStyle(color: Colors.white70, fontSize: 12),
                      ),
                      Text(
                        "${isAr ? 'المتبقي' : 'Remaining'}: ${summary.remainingAmount.toStringAsFixed(0)} EGP",
                        style: const TextStyle(color: Colors.white70, fontSize: 12),
                      ),
                    ],
                  ),
  ...
  Widget _buildStatsRow(bool isAr, bool isDark, FeesSummaryEntity summary) {
    return Row(
      children: [
        _miniStatCard(
          isAr ? "إجمالي المصاريف" : "Total",
  ...
  Widget _miniStatCard(String label, String val, Color color, IconData icon, bool isDark) {
    return Expanded(
      child: Container(
        padding: const EdgeInsets.symmetric(vertical: 14, horizontal: 8),
        decoration: BoxDecoration(
...
Widget _buildFeeRow(FeeItemEntity fee, bool isAr, Color cardColor, Color textColor, bool isDark) {
  final color = fee.color;
  final statusText = fee.getDisplayStatus(isAr);  // 🔥 من Firebase
    ...
    child: Row(
      children: [
        Container(
          ...
          child: Icon(fee.icon, color: color, size: 20),
        ),
        const SizedBox(width: 14),
        Expanded(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 3),
              Text(
                "${fee.amount.toStringAsFixed(0)} EGP",
                style: const TextStyle(
                  color: Color(0xFF1D4ED8),
                  fontWeight: FontWeight.w900,
                  fontSize: 15,
  ...
  Widget _buildPayButton(bool isAr) {
    return Container(
      width: double.infinity,
      height: 58,
      decoration: BoxDecoration(
